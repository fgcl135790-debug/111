...
with st.expander("⚙️ 設定：輸入美股金鑰 / 更換美股代號 / 模擬測試", expanded=False):
    # 對接 Alpaca 的雙金鑰驗證機制
    alpaca_key_id = st.text_input("Alpaca KEY ID (Client ID)", type="password")
...
# --- 📱 定義手機單頁即時擦除動態容器鎖定 ---
retracement_alert_spot = st.empty() # 獨立轉折警告專用欄位
price_block = st.empty()  
threshold_spot = st.empty()
signal_spot = st.empty()
st.markdown("<hr>", unsafe_allow_html=True)

st.markdown("<b style='font-size:14px; color:#ddd;'>📋 最佳買賣報價 (Top of Book)</b>", unsafe_allow_html=True)
...
def process_market_logic(current_price, big_order_vol, stock_name):
    ...
    if recent_sell_cnt > recent_buy_cnt:
        decision_text = f"⏳ 偵測中：未出現30秒內連續3筆精確內盤大單 ({big_order_vol}股)，保持觀望..."
    else:
        decision_text = f"⏳ 偵測中：未出現30秒內連續3筆精確外盤大單 ({big_order_vol}股)，保持觀望..."
        
    # 訊號發射：只認 30s 大戶單方向連續出手頻率，滿足 3 次瞬發燈號
    if recent_buy_cnt >= 3:
...
# --- API 連線與測試模式控制機制 (切換為 Alpaca 美股規格) ---
if (alpaca_key_id and alpaca_secret_key) or test_mode:
    # 串接 Alpaca 免費的實時數據伺服器
    ...
    def start_streaming(code):
        try:
            ...
            # =========================================================================
            # 📌 模擬劇本模式 (美股單價高位優化 + 成交五檔完美對齊)
            # =========================================================================
            if test_mode:
                open_price = 120.00
                stock_name = code
                total_volume_lots = 45000000
                trade_time = int(current_now * 1000)
                cycle = int(current_now) % 40
                
                if cycle < 10:
                    raw_price = 120.05 + (cycle * 0.15)
                    ...
                    raw_price = 122.50 - ((cycle - 10) * 0.20)
                    current_price = round(raw_price, 2)
                    tick_qty = 0
                    tick_price = current_price
                    ...
                    raw_price = 119.80 - ((cycle - 20) * 0.20)
                    current_price = round(raw_price, 2)
                    ...
                    raw_price = 117.20 + ((cycle - 30) * 0.20)
                    current_price = round(raw_price, 2)
                    tick_qty = 0
                    tick_price = current_price
            ...
            # =========================================================================
            # 📌 盤中實時美股 Alpaca 資料串接模式（🟢 網址字串徹底分離，絕不相黏版）
            # =========================================================================
            else:
                try:
                    # 優先嘗試透過 api 物件抓取
            ...
            if "做多" in decision:
                signal_spot.markdown(f"<div style='background-color:#2e1518; padding:8px; border-radius:4px; border-left:5px solid #ff4466; color:#ff4466; font-size:14px; font-weight:bold;'>{decision}</div>", unsafe_allow_html=True)
            elif "做空" in decision:
